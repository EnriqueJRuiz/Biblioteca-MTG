<%@ page session="false" %>
<%@ taglib uri="http://tiles.apache.org/tags-tiles" prefix="tiles"%>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>
<%@ taglib uri="http://www.springframework.org/tags" prefix="spring"%>

	<section class="col-xs-12">
		<div class="col-xs-12  col-md-10 col-md-offset-1 col-lg-6 col-lg-offset-3 jumbotron">
			<div class="row">
				<header class="col-xs-12  col-md-10 col-md-offset-1  col-lg-8 col-lg-offset-2 text-center page-header titulo"><h1></h1></header>
			</div> 
			<div class="row">
				<div class="panel-body">
					<p><h3>Dentro de la aplicacion se podran ver y gestionar las cartas, ampliaciones y los colores de todo lo relacionado con <strong class="titulo">Magic the Gathering</strong></h3></p>
				</div>
			</div>
		</div>
	</section>

